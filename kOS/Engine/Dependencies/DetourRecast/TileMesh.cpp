...
#include "../Include/DetourRecast/TileMesh.h"
...
#include "../Include/DetourRecast/DetourNavMeshBuilder.h"
#include "../Include/DetourRecast/DetourDebugDraw.h"
#include "../Include/DetourRecast/NavMeshTesterTool.h"
#include "../Include/DetourRecast/NavMeshPruneTool.h"
#include "../Include/DetourRecast/OffMeshConnectionTool.h"
//#include "ConvexVolumeTool.h"
//#include "../Include/DetourRecast/CrowdTool.h"
...
void Sample_TileMesh::handleRender()
{
		 ...
		 m_drawMode == DRAWMODE_NAVMESH_INVIS))
	{
		if (m_drawMode != DRAWMODE_NAVMESH_INVIS) {
			duDebugDrawNavMeshWithClosedList(&m_dd, *m_navMesh, *m_navQuery, m_navMeshDrawFlags, renderNavMesh);
		}
...
void Sample_TileMesh::saveTiles(std::string fileName) {
	Sample::saveAll(fileName.c_str(), m_navMesh);
}

bool Sample_TileMesh::loadTiles(std::string fileName) {
	if (m_navMesh) dtFreeNavMesh(m_navMesh);
	m_navMesh = Sample::loadAll(fileName.c_str());
	if (m_navMesh) {
		dtStatus status = m_navQuery->init(m_navMesh, 2048);
		if (status == DT_SUCCESS) {
			handleRender();
			renderNavMesh.CreateMesh();
			return true;
		}
	}
	LOGGING_WARN("Nav Mesh Load Failed: Unable to Load Tiles");
	return false;
}

void Sample_TileMesh::InitdtCrowd() {
	//m_crowd->init()
}
