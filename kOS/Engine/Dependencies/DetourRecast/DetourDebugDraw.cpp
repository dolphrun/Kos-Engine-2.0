...
#include "../Include/DetourRecast/DetourDebugDraw.h"
#include "../Include/DetourRecast/DetourNavMesh.h"
#include "../Include/DetourRecast/DetourCommon.h"
#include "../Include/DetourRecast/DetourNode.h"

inline glm::vec4 ConvertCol(unsigned int col)
{
...
{
	dtPolyRef base = mesh.getPolyRefBase(tile);
	//const unsigned int tileColor = duIntToCol(mesh.decodePolyIdTile(base), 128);
	//glm::vec4 tc = ConvertCol(tileColor); 
	...
	for (int i = 0; i < tile->header->polyCount; i++)
	{
		...
		//unsigned int col =
		//	(query && query->isInClosedList(base | (dtPolyRef)i))
		//	? duRGBA(255, 196, 0, 64)
		//	: ((flags & DU_DRAWNAVMESH_COLOR_TILES)
		//		? tileColor
		//		: duTransCol(dd->areaToCol(p->getArea()), 64));

		//glm::vec4 color = ConvertCol(col); // pack into vec4(0-1)
		...
		for (int j = 0; j < pd->triCount; j++)
		{
			...
			for (int k = 0; k < 3; k++)
			{
				...
				out.triVerts.push_back({ glm::vec3(v[0], v[1], v[2])});
			}
		}
	}
	...
	for (int i = 0; i < tile->header->polyCount; i++)
	{
		...
		{
			int v0 = p->verts[j];
			int v1 = p->verts[(j + 1) % nj];

			const float* a = &tile->verts[v0 * 3];
			const float* b = &tile->verts[v1 * 3];

			out.lineVerts.push_back({ glm::vec3(a[0], a[1], a[2])});
			out.lineVerts.push_back({ glm::vec3(b[0], b[1], b[2])});
		}
	}
	...
	{
		const float* v = &tile->verts[i * 3];
		out.pointVerts.push_back({ glm::vec3(v[0],v[1],v[2])});
	}
}
...
void duDebugDrawNavMeshWithClosedList(struct duDebugDraw* dd, const dtNavMesh& mesh, const dtNavMeshQuery& query, unsigned char flags, DebugNavMesh& nm)
{
	if (!dd) return;
	const dtNavMeshQuery* q = (flags & DU_DRAWNAVMESH_CLOSEDLIST) ? &query : 0;
	...
	for (int i = 0; i < mesh.getMaxTiles(); ++i)
	{
		const dtMeshTile* tile = mesh.getTile(i);
		if (!tile->header) continue;
		drawMeshTile(dd, mesh, q, tile, flags, nm);
	}
}
